// Obtaining an Artifactory server instances in defined in Jenkins:
//just dummy comment line nnmksddkwedky
			
def server = Artifactory.server 'Artifactory Version 4.15.0'

		 //If artifactory is not defined in Jenkins, then create on:
		// def server = Artifactory.newServer url: 'Artifactory url', username: 'username', password: 'password' check
...
pipeline {
    ...
    stages {
        stage('Clone sources'){
            steps {
                git url: 'https://github.com/ellurukalyan/CI-CD'
            }
        }
...
//			timeout(time: 1, unit: 'HOURS') {
			//Parameter indicates wether to set pipeline to UNSTABLE if Quality Gate failed
